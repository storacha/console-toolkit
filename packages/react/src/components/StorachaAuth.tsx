    ...
    async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        if (!controller.signal.aborted) {
          ...
          const registerError = new Error(`failed to register: ${error?.message || String(error)}`)
          throw registerError
        }
      } finally {
        setSubmitted(false)
...
export const StorachaAuthForm = ({ 
  ...
  const [{ handleRegisterSubmit, submitted }] = useStorachaAuthContext()
...
export const StorachaAuthEmailInput = ({ className, style, ...props }: StorachaAuthEmailInputProps) => {
  const [{ email }, { setEmail }] = useStorachaAuthContext()
...
export const StorachaAuthCancelButton = ({ className, style, ...props }: StorachaAuthCancelButtonProps) => {
  const [, { cancelLogin }] = useStorachaAuthContext()
...
export const StorachaAuthSubmitted = ({ 
  ...
  const [{ email }] = useStorachaAuthContext()
...
export const StorachaAuthEnsurer = ({ 
  ...
  const [{ submitted, client, isAuthenticated, isIframe }] = useStorachaAuthContext()
...
export function useStorachaAuthContext(): StorachaAuthContextValue {
